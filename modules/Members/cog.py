...
class Member(commands.Cog):
    ...
    async def register_color(self, interaction: discord.Interaction, color: str):
        ...
        if not re.match(color_str_pattern2, color):
            await interaction.response.send_message("Please input the hexcode for your color in #000000 format", ephemeral=True)
            return
    ...
    async def register(self, interaction: discord.Interaction, name: str, birthday: str, color: str = None):
        ...
        member_join_date = interaction.user.joined_at.astimezone(pytz.timezone('US/Eastern')).strftime('%m/%d/%Y at %I:%M:%S %p')
